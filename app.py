...
# ============================================================
# DATABASE (migrates old qty/sl/tp schema)
# ============================================================
def get_connection():
...
def initialize_database():
    ...
    # Migrate old account table: account(balance, realized_pnl, cycle, status)
    if table_exists(conn, "account"):
        old_cols = table_columns(conn, "account")
        if "balance" in old_cols:
            row = conn.execute(
                "SELECT balance, realized_pnl, cycle FROM account WHERE status='ACTIVE' ORDER BY id DESC LIMIT 1"
            ).fetchone()
            if row is not None and conn.execute("SELECT account_id FROM accounts WHERE account_id=?", (ACCOUNT_ID,)).fetchone() is None:
                c.execute(
                    """
                    INSERT INTO accounts
                    (account_id, starting_balance, cash_balance, realized_pnl, cycle_number, created_at, updated_at)
                    VALUES (?, ?, ?, ?, ?, ?, ?)
                    """,
                    (ACCOUNT_ID, STARTING_BALANCE, float(row[0]), float(row[1] or 0), int(row[2] or 1), now, now),
                )

    # Migrate old positions: qty/sl/tp
    ...
    if "qty" in pos_cols and "quantity" not in pos_cols:
        ...
        if old is not None and float(old[1] or 0) != 0:
            ...
            c.execute(
                ...
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
                """,
                (ACCOUNT_ID, old[0] or "MIXED", "stocks", side, abs(qty), float(old[2] or 0), now, sl, tp, abs(qty) * float(old[2] or 0)),
            )
        c.execute("DROP TABLE positions_legacy")

    # Migrate old history logs into trades/events as text events
    if table_exists(conn, "history"):
        logs = conn.execute("SELECT cycle, log_text FROM history").fetchall()
        for cycle, text in logs:
            c.execute(
                """
                INSERT INTO account_events (account_id, cycle_number, event_type, message, created_at)
                VALUES (?, ?, ?, ?, ?)
                """,
...
def save_account(cash_balance, realized_pnl):
    conn = get_connection()
    conn.execute(
        "UPDATE accounts SET cash_balance=?, realized_pnl=?, updated_at=? WHERE account_id=?",
...
def save_position(position):
    ...
    if position is None:
        ...
        conn.execute(
            ...
            """,
            (
                ACCOUNT_ID, position["ticker"], position["asset_class"], position["side"],
                float(position["quantity"]), float(position["entry_price"]), str(position["entry_time"]),
                position.get("stop_loss"), position.get("target"), float(position["invested_amount"]),
            ),
        )
    conn.commit()
    conn.close()
...
def save_trade(ticker, asset_class, side, quantity, entry_price, exit_price, entry_time, exit_time, realized_pnl, reason):
    ...
    conn.execute(
        ...
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        """,
        (ACCOUNT_ID, account["cycle_number"], ticker, asset_class, side, float(quantity),
         float(entry_price), float(exit_price), str(entry_time), str(exit_time), float(realized_pnl), 0.0, reason),
    )
    conn.commit()
    conn.close()
...
def save_account_event(event_type, message):
    ...
    conn = get_connection()
    conn.execute(
        "INSERT INTO account_events (account_id, cycle_number, event_type, message, created_at) VALUES (?, ?, ?, ?, ?)",
...
def reset_account_cycle():
    ...
    conn.execute(
        """
        UPDATE accounts
        SET starting_balance=?, cash_balance=?, realized_pnl=?, cycle_number=?, updated_at=?
        WHERE account_id=?
        """,
...
def detect_session_mode(ticker):
    a = detect_asset_class(ticker)
    return {"futures": "CME Futures", "forex": "Forex 24/5", "crypto": "Crypto 24/7"}.get(a, "US Stocks / ETFs - Regular Hours")
...
def futures_session_date(ts):
    d = ts.date()
    return d + timedelta(days=1) if ts.time() >= time(18, 0) else d
...
def normalize_ohlcv(raw, intraday=True):
    ...
    df = df.reset_index()
    o, h, l, c, v = find_column(df, ["Open"]), find_column(df, ["High"]), find_column(df, ["Low"]), find_column(df, ["Close", "Adj Close"]), find_column(df, ["Volume"])
    if None in (o, h, l, c):
        return pd.DataFrame()
...
def apply_session_filter(df, mode):
    ...
    if mode == "US Stocks / ETFs - Regular Hours":
        ...
        out["session_date"] = out["calendar_date"]
...
def get_context_dataframe(revealed, session_date, preset):
    ...
    dates = sorted(revealed["session_date"].unique())
    ...
    return revealed[revealed["session_date"] >= start].copy()
    ...
    for i in range(len(df)):
        ...
        if CHOCH_ENABLE:
            if bias == "BULL" and last_hl is not None and close < last_hl:
                choch.append({"i": i, "dir": "down"}); last_hl = None; bias = "NEUTRAL"
            elif bias == "BEAR" and last_lh is not None and close > last_lh:
                choch.append({"i": i, "dir": "up"}); last_lh = None; bias = "NEUTRAL"
    return bos, choch
        ...
        if pos is not None:
            ...
            if side == "long":
                if sl is not None and float(row["low"]) <= float(sl):
                    ...
                    close_position(ticker, px, row["timestamp"], "(STOP-LOSS)"); add_marker(row["timestamp"], px, "SELL"); break
                if tp is not None and float(row["high"]) >= float(tp):
                    px = max(float(tp), float(row["open"]))
                    close_position(ticker, px, row["timestamp"], "(TARGET HIT)"); add_marker(row["timestamp"], px, "SELL"); break
            else:
                if sl is not None and float(row["high"]) >= float(sl):
                    px = max(float(sl), float(row["open"]))
                    close_position(ticker, px, row["timestamp"], "(STOP-LOSS)"); add_marker(row["timestamp"], px, "COVER"); break
                if tp is not None and float(row["low"]) <= float(tp):
                    px = min(float(tp), float(row["open"]))
                    close_position(ticker, px, row["timestamp"], "(TARGET HIT)"); add_marker(row["timestamp"], px, "COVER"); break
...
if source_df.empty:
    ...
    if filtered_df.empty:
        st.sidebar.error("No bars matched the selected session.")
    else:
        dates = sorted(filtered_df["session_date"].unique())
        st.sidebar.success(f"{len(filtered_df):,} session bars\n{dates[0]} → {dates[-1]}")
        key = f"practice_date_{ticker}_{timeframe}_{effective_session_mode}"
        if key not in st.session_state or st.session_state[key] not in dates:
            st.session_state[key] = dates[-3] if len(dates) >= 3 else dates[-1]
        practice_date = st.sidebar.date_input("Practice Session", min_value=dates[0], max_value=dates[-1], key=key)
        rows = filtered_df[filtered_df["session_date"] == practice_date]
        can_start = not rows.empty
        if rows.empty:
            st.sidebar.warning("No bars for this session.")
        elif TIMEFRAME_CONFIG[timeframe]["intraday"]:
            labels = rows["timestamp"].dt.strftime("%H:%M").tolist()
            sk = f"start_time_{ticker}_{timeframe}_{practice_date}"
            lab = st.sidebar.selectbox("Replay Start Time", labels, index=min(10, len(labels) - 1), key=sk)
            selected_start_timestamp = rows[rows["timestamp"].dt.strftime("%H:%M") == lab]["timestamp"].iloc[0]
        else:
            selected_start_timestamp = rows["timestamp"].iloc[0]
...
st.sidebar.caption("Follow ON + continuous: last 220 bars. Follow ON + stocks: current session. Follow OFF: full context, Y-axis fits all visible bars.")
...
st.sidebar.markdown("---")
if st.sidebar.button("🚀 Start / Reset Replay", disabled=(filtered_df.empty or not can_start)):
...
if not st.session_state.sim_active or st.session_state.df.empty:
    st.info("Choose ticker, timeframe, session, date, start time, then Start / Reset Replay.")
    st.stop()
...
current_session = cur["session_date"]
...
chart_df = get_context_dataframe(calc, current_session, context_preset).reset_index(drop=True)
if chart_df.empty:
...
c3.metric("Position Value", f"${pos_val:.2f}")
c4.metric("Equity", f"${equity:.2f}", f"${equity - float(acc['starting_balance']):.2f}")
c5.metric("Realized P&L", f"${realized:.2f}")
c6.metric("Unrealized P&L", f"${unrl:.2f}")
...
if show_struct and not sdf.empty:
    ...
    for e in bos[-MAX_BOS_LABELS:]:
        if 0 <= e["i"] < len(sdf):
            ...
            if e["dir"] == "up":
                badge(fig, x, float(sdf["high"].iloc[e["i"]]), "BOS↑", C_BOS_UP, 22, True)
            else:
                badge(fig, x, float(sdf["low"].iloc[e["i"]]), "BOS↓", C_BOS_DN, -22, True)
    for e in choch[-MAX_CHOCH_LABELS:]:
        if 0 <= e["i"] < len(sdf):
            x = sdf["x_index"].iloc[e["i"]]
            if e["dir"] == "up":
                badge(fig, x, float(sdf["high"].iloc[e["i"]]), "CHoCH↑", C_CH_UP, 22, True)
            else:
                badge(fig, x, float(sdf["low"].iloc[e["i"]]), "CHoCH↓", C_CH_DN, -22, True)
...
if show_zones and confirmed:
    ...
    for z in zones:
        ...
        fig.add_annotation(x=rx, y=z["mid"], row=1, col=1, xanchor="left", text=f"<b> {z['mid']:.2f} {tag} {z['low_touches']}L/{z['high_touches']}H</b>", showarrow=False, font=dict(size=10, color="#fff"), bgcolor=color)
...
for m in st.session_state.markers:
    ...
    fig.add_trace(go.Scatter(x=[ix[-1]], y=[m["price"]], mode="markers", showlegend=False, marker=dict(symbol=sym, size=12, color=col, line=dict(color="#fff", width=1))), row=1, col=1)
...
if follow_replay and continuous:
    start_idx = max(0, current_x - (CONTINUOUS_FOLLOW_BARS - 1))
    end_idx = min(len(chart_df) - 1, current_x + 8)
    follow_title = f"Follow: {CONTINUOUS_FOLLOW_BARS} bars"
elif follow_replay:
    day = chart_df[chart_df["session_date"] == current_session]
...
cam_sig = (context_preset, follow_replay, timeframe, current_session, active_mode)
...
for r in (1, 2):
    fig.update_xaxes(type="linear", tickmode="array", tickvals=ticks, ticktext=tick_text, gridcolor="#1f2937", rangeslider_visible=False, showspikes=True, spikemode="across", row=r, col=1)
...
if apply_camera:
    ...
    if pr: fig.update_yaxes(range=pr, autorange=False, row=1, col=1)
    if vr: fig.update_yaxes(range=vr, autorange=False, row=2, col=1)
...
with right:
    ...
    for lab, n in btns:
        if st.button(lab):
            ...
            st.rerun()
    st.markdown("---")
    st.success("🟢 LONG") if signed > 0 else st.error("🔴 SHORT") if signed < 0 else st.info("FLAT")
    st.caption(f"Session: {current_session}")
    st.caption(f"Visible: {len(visible):,}")
    st.caption(f"Rendered: {len(chart_df):,}")
    st.caption(f"Carry: {'ON' if st.session_state.active_carry_mode else 'OFF'}")
    ...
    with d:
        ...
        if st.button("💾 Update SL/TP"):
            ...
            if pos["side"] == "long" and nsl >= current_price:
                st.error("Long stop must be below price."); ok = False
            if pos["side"] == "short" and nsl <= current_price:
                st.error("Short stop must be above price."); ok = False
            if ok:
...
with st.expander("📘 Replay Information"):
    st.markdown(
        ...
        "Start / Reset Replay does not reset the wallet."
    )
