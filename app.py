...
#
# FEATURES
# - Session-compressed x-axis (no calendar gaps)
...
FOLLOW_WINDOW_BARS = 220

CONTINUOUS_ASSET_CLASSES = {"futures", "forex", "crypto"}
...
def normalize_ohlcv(raw, intraday=True):
    ...
    if isinstance(dataframe.columns, pd.MultiIndex):
        dataframe.columns = [str(col[0]) for col in dataframe.columns]
    ...
    output = pd.DataFrame({
        ...
        "volume": dataframe[volume_col] if volume_col is not None else 0.0,
    })
    ...
    output = output.drop_duplicates(subset=["timestamp"])
    ...
    output = output[output["bar_ratio"] <= 3.0].copy()

    if output.empty:
        return pd.DataFrame()

    rolling_med = output["close"].rolling(80, min_periods=10).median().shift(1)
    expanding_med = output["close"].expanding(min_periods=1).median()
    reference = rolling_med.fillna(expanding_med).replace(0, np.nan)
    price_ratio = output["close"] / reference

    if intraday:
        output = output[(price_ratio >= 0.10) & (price_ratio <= 10.0)].copy()
    else:
        output = output[(price_ratio >= 0.02) & (price_ratio <= 50.0)].copy()

    if output.empty:
        return pd.DataFrame()

    output = output.drop(columns=["bar_ratio"], errors="ignore")
    output["calendar_date"] = output["timestamp"].dt.date
    output["label"] = output["timestamp"].dt.strftime("%m-%d %H:%M")
...
def apply_session_filter(dataframe, ticker, session_mode):
    ...
    if session_mode == "US Stocks / ETFs - Regular Hours":
        output = output[output["timestamp"].dt.weekday < 5].copy()
        output = output[~output["calendar_date"].apply(is_us_market_holiday)].copy()

        minutes_since_midnight = (
            output["timestamp"].dt.hour * 60 + output["timestamp"].dt.minute
        )
        ...
        sunday_evening = (weekday == 6) & (current_time >= time(18, 0))
        output = output[valid_weekday | sunday_evening].copy()

        current_time = output["timestamp"].dt.time
        in_break = (current_time >= time(17, 0)) & (current_time < time(18, 0))
        output = output[~in_break].copy()

        output["session_date"] = output["timestamp"].apply(futures_session_date)
        ...
        valid_weekday = weekday < 5
        sunday_open = (weekday == 6) & (current_time >= time(17, 0))
        friday_after_close = (weekday == 4) & (current_time >= time(17, 0))

        output = output[
            (valid_weekday | sunday_open) & ~friday_after_close
        ].copy()
        output["session_date"] = output["calendar_date"]
...
def apply_custom_session(dataframe, start_time, end_time, include_weekends):
    if dataframe.empty:
        return dataframe
...
def fetch_history(ticker, timeframe):
    ...
    periods = [config["period"], config["fallback_period"]]
    ...
    for period in periods:
        try:
            ...
            normalized = normalize_ohlcv(raw, intraday=config["intraday"])
...
def get_context_dataframe(revealed_dataframe, practice_date, preset):
    if revealed_dataframe.empty:
        return revealed_dataframe.copy()

    available_dates = sorted(revealed_dataframe["session_date"].unique())

    if practice_date not in available_dates:
        return revealed_dataframe.copy()

    current_index = available_dates.index(practice_date)
    ...
    start_index = max(0, current_index - previous_days)
...
def get_price_axis_range(dataframe):
    ...
    for col in ["low", "high", "fast_ma", "slow_ma"]:
        if col in dataframe.columns:
            values.extend(
                pd.to_numeric(dataframe[col], errors="coerce").dropna().tolist()
            )
    ...
    median_price = float(dataframe["close"].median())
    spread = max(high_value - low_value, max(median_price * 0.003, 0.10))
...
def open_position(ticker, price, timestamp, side, amount, stop_loss, target):
    account = load_account()
    ...
    save_account(new_cash, float(account["realized_pnl"]))
...
def close_position(ticker, price, timestamp, reason="", quantity_to_close=None):
    position = load_position()
    if position is None:
        return
    ...
    quantity_to_close = min(float(quantity_to_close), total_quantity)
    entry_price = float(position["entry_price"])
    price = float(price)

    if position["side"] == "long":
        realized = (price - entry_price) * quantity_to_close
        cash_change = price * quantity_to_close
        new_cash = float(account["cash_balance"]) + cash_change
    else:
        realized = (entry_price - price) * quantity_to_close
        cash_change = -price * quantity_to_close
        new_cash = float(account["cash_balance"]) + cash_change

    new_realized = float(account["realized_pnl"]) + realized
    save_account(new_cash, new_realized)
...
def advance_bars(number_of_bars):
    ...
    for _ in range(int(number_of_bars)):
        ...
        row = dataframe.iloc[st.session_state.step]
        ...
        if position is not None:
            ...
            if side == "long":
                if stop_loss is not None and float(row["low"]) <= float(stop_loss):
                    execution_price = min(float(stop_loss), float(row["open"]))
                    close_position(current_ticker, execution_price, timestamp, "(STOP-LOSS)")
                    add_marker(timestamp, execution_price, "SELL")
                    st.toast(f"Long stopped at ${execution_price:.2f}", icon="\U0001f4a5")
                elif target is not None and float(row["high"]) >= float(target):
                    execution_price = max(float(target), float(row["open"]))
                    close_position(current_ticker, execution_price, timestamp, "(TARGET HIT)")
                    add_marker(timestamp, execution_price, "SELL")
                    st.toast(f"Long target at ${execution_price:.2f}", icon="\U0001f389")

            else:
                if stop_loss is not None and float(row["high"]) >= float(stop_loss):
                    execution_price = max(float(stop_loss), float(row["open"]))
                    close_position(current_ticker, execution_price, timestamp, "(STOP-LOSS)")
                    add_marker(timestamp, execution_price, "COVER")
                    st.toast(f"Short stopped at ${execution_price:.2f}", icon="\U0001f4a5")
                elif target is not None and float(row["low"]) <= float(target):
                    execution_price = min(float(target), float(row["open"]))
                    close_position(current_ticker, execution_price, timestamp, "(TARGET HIT)")
                    add_marker(timestamp, execution_price, "COVER")
                    st.toast(f"Short target at ${execution_price:.2f}", icon="\U0001f389")

        # Force-close at session boundary if carry is OFF
        if not st.session_state.active_carry_mode and position is not None:
    ...
    if current_position is None:
        current_row = dataframe.iloc[st.session_state.step]
        maybe_reset_depleted_account(float(current_row["close"]))
...
custom_start_time = time(hour=9, minute=30)
custom_end_time = time(hour=16, minute=0)
custom_include_weekends = False

if effective_session_mode == "Custom Session":
    custom_start_time = st.sidebar.time_input("Custom session start", value=time(9, 30))
    custom_end_time = st.sidebar.time_input("Custom session end", value=time(16, 0))
    custom_include_weekends = st.sidebar.checkbox("Include weekends", value=False)

st.sidebar.caption(
    f"Yahoo native interval: `{TIMEFRAME_CONFIG[timeframe]['yf_interval']}`"
)
...
if source_df.empty:
    ...
    filtered_df = apply_session_filter(source_df, ticker, effective_session_mode)

    if effective_session_mode == "Custom Session":
        filtered_df = apply_custom_session(
            filtered_df, custom_start_time, custom_end_time, custom_include_weekends
        )

    if filtered_df.empty:
        st.sidebar.error(
            "Data was returned, but no bars matched the selected session."
        )
    else:
        available_dates = sorted(filtered_df["session_date"].unique())
        ...
        practice_date_key = (
            f"practice_date_{ticker}_{timeframe}_{effective_session_mode}"
        )
        ...
        if (
            ...
            st.session_state[practice_date_key] = default_date
        ...
        if practice_rows.empty:
            st.sidebar.warning("No bars exist for this session.")
        elif TIMEFRAME_CONFIG[timeframe]["intraday"]:
            labels = practice_rows["timestamp"].dt.strftime("%H:%M").tolist()
            default_index = min(10, len(labels) - 1)
            ...
            selected_start_timestamp = practice_rows[
                practice_rows["timestamp"].dt.strftime("%H:%M") == selected_label
            ]["timestamp"].iloc[0]
        else:
...
st.sidebar.markdown("---")
st.sidebar.subheader("\U0001f4e6 Position Behavior")
...
st.sidebar.caption(
    "When disabled, open positions are closed at the "
    "final bar of each session boundary."
)
...
st.sidebar.markdown("---")
st.sidebar.subheader("\U0001f4c8 Moving Averages")
...
fast_length = st.sidebar.number_input(
    "Fast MA length", min_value=2, max_value=500, value=int(default_fast), step=1
)

slow_length = st.sidebar.number_input(
    "Slow MA length", min_value=2, max_value=500, value=int(default_slow), step=1
)

show_volume_ma = st.sidebar.checkbox("Show volume MA", value=True)

volume_ma_length = st.sidebar.number_input(
    "Volume MA length", min_value=2, max_value=200, value=20, step=1
)
...
st.sidebar.markdown("---")
st.sidebar.subheader("\U0001f4f7 Chart Context")
...
st.sidebar.caption(
    "X-axis uses sequential session bars (no calendar gaps).\n\n"
    "Follow ON + continuous (futures/forex/crypto): "
    f"last {FOLLOW_WINDOW_BARS} bars.\n"
...
st.title("\U0001f4b9 Market Replay Simulator")

st.caption(
    "Session-compressed replay \u2022 persistent paper account \u2022 "
    "native Yahoo candles \u2022 220-bar follow window"
)
...
revealed_df = dataframe.iloc[: step + 1].copy()
...
if show_ma:
    calc_df["fast_ma"] = moving_average(calc_df["close"], fast_length, ma_type)
    calc_df["slow_ma"] = moving_average(calc_df["close"], slow_length, ma_type)

if show_volume_ma:
    calc_df["volume_ma"] = calc_df["volume"].rolling(int(volume_ma_length)).mean()

chart_df = get_context_dataframe(
    calc_df, practice_date, context_preset
).reset_index(drop=True)

if chart_df.empty:
    chart_df = calc_df.copy().reset_index(drop=True)

# Compressed x-axis: sequential integer per bar
...
if position is None:
    ...
    if position["side"] == "long":
        ...
        unrealized_pnl = (current_price - float(position["entry_price"])) * quantity
        position_type = "\U0001f7e2 LONG"
    else:
        quantity_signed = -quantity
        position_value = -quantity * current_price
        unrealized_pnl = (float(position["entry_price"]) - current_price) * quantity
        position_type = "\U0001f534 SHORT"

    position_text = f"{abs(quantity_signed):.4f} units"

equity = cash + position_value
total_pnl = equity - float(account["starting_balance"])
...
m2.metric("Equity", f"${equity:.2f}", f"${total_pnl:.2f}")
m3.metric("Cash", f"${cash:.2f}")
m4.metric(f"Position ({position_type})", position_text)
m5.metric("Realized P&L", f"${realized_pnl:.2f}")
m6.metric("Unrealized P&L", f"${unrealized_pnl:.2f}")

st.caption(
    f"Account cycle {account['cycle_number']} \u2022 "
    f"Starting ${account['starting_balance']:.2f} \u2022 "
    f"Session: {st.session_state.active_session_mode}"
)
...
if show_ma and "fast_ma" in chart_df.columns:
    figure.add_trace(
        go.Scatter(
            ...
            line=dict(color="#3b82f6", width=1.7),
        ),
        row=1,
        col=1,
    )
...
if show_volume_ma and "volume_ma" in chart_df.columns:
    figure.add_trace(
        go.Scatter(
            ...
            name=f"VolMA {int(volume_ma_length)}",
            line=dict(color="#ff6d00", width=1.7),
        ),
        row=2,
        col=1,
    )
...
for drawing in st.session_state.drawings:
    if drawing["type"] == "hline":
        ...
        x0_matches = chart_df.index[chart_df["timestamp"] == drawing["x0"]].tolist()
        x1_matches = chart_df.index[chart_df["timestamp"] == drawing["x1"]].tolist()
        if x0_matches and x1_matches:
            figure.add_trace(
                go.Scatter(
                    x=[x0_matches[-1], x1_matches[-1]],
...
asset_class = detect_asset_class(st.session_state.active_ticker)
is_continuous = asset_class in CONTINUOUS_ASSET_CLASSES
...
if st.session_state.camera_signature != context_signature:
    st.session_state.camera_signature = context_signature
    st.session_state.camera_revision += 1
    st.session_state.force_camera = True
...
if follow_replay:
    if is_continuous:
        # 220-bar sliding window ending at current bar
        window_start = max(0, current_x - FOLLOW_WINDOW_BARS + 1)
        window_end = current_x
        focus_df = chart_df.iloc[window_start : window_end + 1]
    else:
        # Stocks/ETFs: zoom into current practice session
        session_df = chart_df[chart_df["session_date"] == practice_date]
        if session_df.empty:
            session_df = chart_df
        window_start = int(session_df.index[0])
        window_end = int(session_df.index[-1])
        focus_df = chart_df.iloc[window_start : window_end + 1]
else:
    window_start = 0
    window_end = len(chart_df) - 1
    focus_df = chart_df

# --- X-axis range ---

x_range = [window_start, max(window_start + 1, window_end)]

# --- Tick labels for visible window ---

visible_indices = list(range(window_start, window_end + 1))
num_ticks = min(12, len(visible_indices))

if num_ticks > 0:
    if len(visible_indices) <= num_ticks:
        tick_positions = visible_indices
    else:
        step_size = len(visible_indices) / num_ticks
        tick_positions = [
            visible_indices[int(i * step_size)]
...
tick_labels = [
    pd.Timestamp(chart_df.iloc[int(idx)]["timestamp"]).strftime("%m-%d %H:%M")
    for idx in tick_positions
]

# --- Y-axis ranges from visible bars ---

price_range = get_price_axis_range(focus_df)
volume_range = get_volume_axis_range(focus_df)
...
figure.update_layout(
    ...
    uirevision=(
        ...
        if follow_replay
        else f"manual-{st.session_state.camera_revision}"
    ),
)
...
if volume_range is not None:
    figure.update_yaxes(range=volume_range, row=2, col=1)
...
with control_col:
    st.markdown("#### \u23f1\ufe0f")
    st.caption(current_timestamp.strftime("%H:%M"))
    st.caption(f"Native {st.session_state.active_interval}")
    ...
    st.caption(f"Visible bars: {window_end - window_start + 1}")
    st.caption(f"Total rendered: {len(chart_df):,}")
    st.caption(
        f"Carry: {'ON' if st.session_state.active_carry_mode else 'OFF'}"
    )
    st.caption(
        f"Asset: {asset_class} "
...
if position is None:
    st.markdown("### \U0001f3ae Open a Position")

    e1, e2, e3, e4, e5 = st.columns([1.2, 1.2, 1.2, 1, 1])

    max_trade = max(10.0, float(cash))
    ...
    with e4:
        st.write("")
        st.write("")
        if st.button("\U0001f7e2 BUY (Long)", width="stretch"):
            valid = True
            if stop_input >= current_price:
                st.error("Long stop must be below current price.")
                valid = False
            if target_input != 0 and target_input <= current_price:
            ...
            if valid:
                open_position(
                    ...
                    target=float(target_input) if target_input > 0 else None,
                )
                add_marker(current_timestamp, current_price, "LONG")
                st.rerun()

    with e5:
        st.write("")
        st.write("")
        if st.button("\U0001f53b SHORT", width="stretch"):
            valid = True
            if stop_input <= current_price:
                st.error("Short stop must be above current price.")
                valid = False
            if target_input != 0 and target_input >= current_price:
            ...
            if valid:
                ...
                add_marker(current_timestamp, current_price, "SHORT")
                st.rerun()
    ...
    st.markdown(f"### \U0001f3ae Manage {side_label} Position")
    ...
    with m4:
        st.write("")
        st.write("")
        if st.button("\U0001f4be Update SL/TP", width="stretch"):
            valid = True
            if position_side == "long" and new_stop >= current_price:
                st.error("Long stop must be below current price.")
                valid = False
            if position_side == "short" and new_stop <= current_price:
                st.error("Short stop must be above current price.")
                valid = False
            ...
            if valid:
                ...
                position["target"] = (
                    float(new_target) if new_target > 0 else None
                )
                save_position(position)
                st.rerun()
    ...
    with m5:
        st.write("")
        st.write("")
        close_label = "\U0001f534 SELL" if position_side == "long" else "\U0001f7e2 COVER"
        ...
        if st.button(close_label, width="stretch"):
            if close_quantity > 0:
                ...
                add_marker(
                    current_timestamp,
                    current_price,
                    "SELL" if position_side == "long" else "COVER",
                )
                maybe_reset_depleted_account(current_price)
                st.rerun()
